<?php
		...
		if($_GET['do'] == "delete"){
			$id = $_GET['id'];
			$result = $con->query("delete from fuzhu where piaojuid in ($id);");
			if($result){
	...
	if($_POST['Submit']){
		$id = $_POST['id'];
		$statu1 = $_POST['statu1'];
		$result = $con->query("update fuzhu set statu1='$statu1'where piaojuid='$id';");
		if($result){
		...
		<div class="div_from_aoto" style="width: 800px; margin:30px 40px;">
			...
			<form name="form1" method="post" action="">
				<table width="1000" border="0" cellspacing="0" cellpadding="0">
					...
					<?php 
							$result = mysqli_query($con,"select * from fuzhu;");
							while($row = mysqli_fetch_assoc($result)){
					?>
					<tr>
						<td width="200" valign="middle"><input style="width:15px;" name="checkboxitem" type="checkbox" value="<?php echo $row['piaojuid']; ?>"><?php echo $row['piaojuid']; ?></td>
						<td width="200"><?php echo $row['chufangid']; ?></td>
						<td width="200"><?php echo $row['bingname']; ?></td>
						<td width="200"><?php echo $row['fuzhu']; ?></td>
						<td width="200"><?php echo $row['didain']; ?></td>
						<td width="200"><?php echo $row['money']; ?></td>
						<td width="200"><?php echo $row['statu1']; ?></td>
						<td width="200"><?php echo $row['statu2']; ?></td>
						<td width="*">
							<input style="width:60px; height:22px;" value="修改" type="button" onClick="location.href='?do=change&id=<?php echo $row['piaojuid']; ?>'">
						</td>
						<td width="34">
							<input style="width:60px; height:22px;" value="删除" type="button" onClick="location.href='?do=delete&id=<?php echo $row['piaojuid']; ?>'">
						</td>
						<td width="34">
							<input style="width:60px; height:22px;" value="支付" type="button" onClick="location.href='code/code.php'">
						</td>
					</tr>
					<?php }?>
					<tr>
			...
			<?php
				if($_GET['do'] == "change"){
					$id = $_GET['id'];
					$result = mysqli_query($con,"select * from fuzhu where piaojuid='$id';");
